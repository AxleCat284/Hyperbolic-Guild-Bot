...
const events = readdirSync('./src/music/events/').filter(file => file.endsWith('.js'));
...
readdirSync('./src/music/commands/').forEach(dirs => {
    const commands = readdirSync(`./src/music/commands/${dirs}`).filter(files => files.endsWith('.js'));
